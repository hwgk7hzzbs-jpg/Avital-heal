...
export default {
  async fetch(request, env) {
    ...
    if (path.match(/^\/api\/contacts\/\d+$/) && method === 'PUT') {
      const id = path.split('/').pop();
      return handleUpdateContact(id, request, env);
    }
    if (path.match(/^\/api\/contacts\/\d+$/) && method === 'DELETE') {
      const id = path.split('/').pop();
      return handleDeleteContact(id, env);
    }
...
async function handleStats(env) {
  try {
    ...
    const newContacts = await env.DB.prepare('SELECT COUNT(*) as count FROM contacts WHERE status = \'new\'').first();
    const recentContacts = await env.DB.prepare('SELECT id, full_name, phone, email, message, status, created_at FROM contacts ORDER BY created_at DESC LIMIT 5').all();
...
async function handleContactSubmission(request, env) {
  try {
    ...
    if (turnstileToken && env.TURNSTILE_SECRET_KEY) {
      ...
      if (!valid) {
        return errorResponse('אימות CAPTCHA נכשל', 403);
      }
    }
